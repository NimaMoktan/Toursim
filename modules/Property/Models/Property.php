...
class Property extends Bookable
{
    ...
    public function search(Request $request)
    {
        ...
        if ($agentIds = $request->agentIds) {
            $query->whereIn('author_id', $agentIds);
        }
        if (!empty($location_id = $request->query('location_id'))) {
            ...
            if (!empty($location)) {
                $query->join('bravo_locations', function ($join) use ($location) {
                    $join->on('bravo_locations.id', '=', 'bc_properties.location_id')
        ...
        if (!empty($property_type = $request->query("property_type"))) {
            if($property_type == 3){
                $query->where("bc_properties.is_sold", 1);
            }else {
    ...
    {
        if ($this->property_type == self::FOR_SALE) {
            return __('For Buy');
        } else {
            return __('For Rent');
        }
